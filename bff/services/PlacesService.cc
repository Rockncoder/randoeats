...
int priceLevelToInt(const std::string& enumName) {
    if (enumName == "PRICE_LEVEL_FREE")
        return 0;
    if (enumName == "PRICE_LEVEL_INEXPENSIVE")
        return 1;
    if (enumName == "PRICE_LEVEL_MODERATE")
        return 2;
    if (enumName == "PRICE_LEVEL_EXPENSIVE")
        return 3;
    if (enumName == "PRICE_LEVEL_VERY_EXPENSIVE")
        return 4;
...
bool hasParking(const Json::Value& place) {
    if (!place.isMember("parkingOptions"))
        return false;
...
Json::Value PlacesService::normalize(const Json::Value& place) {
    ...
    if (place.isMember("rating"))
        out["rating"] = place["rating"].asDouble();
    ...
    if (place.isMember("currentOpeningHours")) {
        ...
        if (hours.isMember("openNow"))
            out["openNow"] = hours["openNow"].asBool();
    ...
    if (place.isMember("servesBeer"))
        out["servesBeer"] = place["servesBeer"].asBool();
    if (place.isMember("servesWine"))
        out["servesWine"] = place["servesWine"].asBool();
    ...
    if (place.isMember("parkingOptions"))
        out["hasParking"] = hasParking(place);
...
drogon::Task<ServiceResult> PlacesService::nearby(NearbyQuery q) {
    const bool usesAtmosphere = q.beer || q.wine || q.patio || q.group || q.parking;

    // Cache key covers every input that changes the result set.
    std::string priceKey;
    for (int level : q.priceLevels)
        priceKey += std::to_string(level);
    std::array<char, 256> key{};
    std::snprintf(key.data(),
                  key.size(),
                  "nearby:%.3f:%.3f:%d:%d:%s:o%d:r%.1f:p%s:%d%d%d%d%d",
                  q.lat,
                  q.lng,
                  q.radius,
                  q.maxResults,
    ...
    if (up.status == 0) {
        co_return ServiceResult{502, errorBody("upstream unavailable"), "MISS", up.upstreamMs, 0};
    }
    if (up.status != 200) {
        co_return ServiceResult{502, errorBody("upstream error"), "MISS", up.upstreamMs, 0};
    }
    ...
    for (const auto& place : up.json["places"]) {
        ...
        if (q.beer && !flagTrue(place, "servesBeer"))
            continue;
        if (q.wine && !flagTrue(place, "servesWine"))
            continue;
        if (q.patio && !flagTrue(place, "outdoorSeating"))
            continue;
        if (q.group && !flagTrue(place, "goodForGroups"))
            continue;
        if (q.parking && !hasParking(place))
            continue;
...
drogon::Task<ServiceResult> PlacesService::details(std::string placeId) {
    ...
    if (up.status == 404) {
        co_return ServiceResult{404, errorBody("not found"), "MISS", up.upstreamMs, 0};
    }
    if (up.status != 200) {
        co_return ServiceResult{502, errorBody("upstream error"), "MISS", up.upstreamMs, 0};
    }
