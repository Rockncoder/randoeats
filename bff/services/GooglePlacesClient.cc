...
GooglePlacesClient::GooglePlacesClient(std::string apiKey) : apiKey_(std::move(apiKey)) {}
...
drogon::Task<UpstreamResult> GooglePlacesClient::searchText(SearchParams p) {
    ...
    Json::Value base;
    base["textQuery"] = p.query.empty() ? "restaurant" : (p.query + " restaurant");
    ...
    if (p.openNow)
        base["openNow"] = true;
    if (p.minRating > 0)
        base["minRating"] = p.minRating;
    for (int level : p.priceLevels) {
        const std::string e = priceLevelEnum(level);
        if (!e.empty())
            base["priceLevels"].append(e);
    }

    std::string mask = kNearbyMask;
    if (p.includeAtmosphere)
        mask += kNearbyAtmosphereMask;
    mask += ",nextPageToken";
    ...
    try {
        ...
        do {
            ...
            try {
                ...
                if (status != 200)
                    break;  // keep any pages already collected
...
drogon::Task<UpstreamResult> GooglePlacesClient::getDetails(std::string placeId) {
    auto req = drogon::HttpRequest::newHttpRequest();
...
drogon::Task<PhotoResult> GooglePlacesClient::getPhoto(std::string photoName, int maxWidth) {
    PhotoResult result;
    const auto start = std::chrono::steady_clock::now();
    try {
        ...
        const std::string path = pathStart == std::string::npos ? "/" : photoUri.substr(pathStart);
