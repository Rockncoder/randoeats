...
void recordMetrics(const drogon::HttpRequestPtr& req,
...
drogon::Task<HttpResponsePtr> RestaurantController::nearby(drogon::HttpRequestPtr req) {
    ...
    q.radius = std::clamp(
        params.count("radius") ? std::stoi(req->getParameter("radius")) : 5000, 1, 50000);
    q.maxResults =
        std::clamp(params.count("max") ? std::stoi(req->getParameter("max")) : 20, 1, 60);
    if (params.count("q"))
        q.query = req->getParameter("q");
    ...
    if (params.count("price")) {  // CSV of 1..4, e.g. "1,2,3"
        ...
        while (std::getline(ss, tok, ',')) {
            if (!tok.empty())
                q.priceLevels.push_back(std::stoi(tok));
        }
    }
    ...
    const ServiceResult result = co_await AppContext::instance().places->nearby(std::move(q));
    recordMetrics(req, result.cache, result.upstreamMs, "nearby_search");
    co_return jsonResponse(result);
}

drogon::Task<HttpResponsePtr> RestaurantController::details(drogon::HttpRequestPtr req,
                                                            std::string placeId) {
    const ServiceResult result = co_await AppContext::instance().places->details(placeId);
    recordMetrics(req, result.cache, result.upstreamMs, "place_details");
    co_return jsonResponse(result);
}

drogon::Task<HttpResponsePtr> RestaurantController::photo(drogon::HttpRequestPtr req,
                                                          std::string /*placeId*/) {
    const auto& params = req->getParameters();
    ...
    int maxWidth = params.count("max_width") ? std::stoi(req->getParameter("max_width")) : 400;
    maxWidth = std::clamp(maxWidth, 1, 1600);

    const PhotoResult photo = co_await AppContext::instance().google->getPhoto(photoRef, maxWidth);
    recordMetrics(req, "NONE", photo.upstreamMs, "photo");

    if (photo.status != 200 || photo.bytes.empty()) {
        co_return errorResponse(photo.status == 404 ? 404 : 502, "photo unavailable");
    }
